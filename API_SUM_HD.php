...
// $sql = "SELECT * FROM activities_join INNER JOIN activities ON activities_join.activities_id=activities.activities_id WHERE student_id = '".$student_id."' AND atttype_id='2'";//ORDER BY คือการเรียงตามลำดับ ID
  $sql = "SELECT SUM(join_unit) AS TotalHD FROM activities_join INNER JOIN activities ON activities_join.activities_id=activities.activities_id WHERE atttype_id = '1' AND student_id = '".$student_id."' "; 
        ...
        if($result->num_rows > 0){
            ...
            while($row = $result->fetch_assoc()) {
                $dbresult[] = array("TotalHD"=>$row["TotalHD"]);
            }
...
$message = $date."||".$sql."\r\n";
$objFopen = @fopen("sql_log_SUM_HD.log", 'a+');
